...
class Kernel_Layer(nn.Module):
    ...
    def _k_means(self, X):
        ...
        """
        ### YOUR CODE HERE
        arr = torch.as_tensor(X)
        features=X.shape[0]
        rand_indices = torch.randint(0,features,(self.hidden_dim,),dtype=torch.long)
        older_cluster = torch.randint(0,self.hidden_dim,(features,),dtype=torch.long) 
        count = 0
        centroids = arr[rand_indices,:]
        while count < 1000:
            count=count+1
            dists = torch.cdist(arr,centroids,p=2)
            current_cluster = dists.argmin(dim=1)
            if current_cluster.eq(older_cluster).all() and count > 1:
                break
            else:
                older_cluster = current_cluster
            for k in range(self.hidden_dim):
                centroids[k,:] = torch.mean(arr[current_cluster.eq(k),:], dim=0)
        if count == 1000:
            print("Could not find best centroids, stopping further iterations..")
        ### END YOUR CODE
        return centroids
    ...
    def forward(self, x):
        ...
        xi = x.unsqueeze(1).expand(size)
        xj = self.prototypes.unsqueeze(0).expand(size)
        #print(xj)
        norm = (xi - xj).pow(2).sum(-1)
        y = torch.exp(-1 * norm / (2 * (self.sigma ** 2)))
        return y 
        ### END YOUR CODE
