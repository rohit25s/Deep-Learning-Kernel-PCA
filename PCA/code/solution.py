...
class AE(nn.Module):
    ...
    def _network(self):
        ...
        self.E = nn.Parameter(data=torch.empty((self.n_features,self.d_hidden_rep)))
    ...
    def _forward(self, X):
		...
		temp = torch.transpose(self.E, 0, 1)
		hidden_output = torch.mm(temp, X)
        return torch.mm(self.D,hidden_output)
        ...
        # Note: here for the network with more layers and nonlinear functions  
        return self.net(X.transpose(0,1)).transpose(0,1)
        ### END YOUR CODE
